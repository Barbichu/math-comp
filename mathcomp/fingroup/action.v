...
Proof. by move=> a; apply: actKin => /[1!inE]. Qed.

Lemma actKV : rev_right_loop invg to.
Proof. by move=> a; apply: actKVin => /[1!inE]. Qed.
...
apply/subsetP/subsetP=> [nSa _ /imsetP[x Sx ->] | nSa x Sx] /[!inE].
  by have /[1!inE] /(mem_imset val) := nSa x Sx; rewrite val_subact sDa.
have /[1!inE] /imsetP[y Sy def_y] := nSa _ (mem_imset val Sx).
...
Proof.
apply/setP => s /[1!inE]; apply/idP/astabP => [sS x|/= S_id].
...
apply/subsetP=> x /[1!inE]; apply: contraLR => R'xa.
...
Proof.
apply/setIidPl; apply/subsetP=> a Da /[1!inE].
...
apply/subsetP=> x Nx /[1!inE]; move/(astabs_act (H :* x)): HDa.
