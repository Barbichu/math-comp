...
Proof. by apply: (big_morph (fun A => A i j)) => [A B|] /[1!mxE]. Qed.
...
Proof.
by rewrite -diag_const_mx diag_mx_sum_delta; apply: eq_bigr => i _ /[1!mxE].
Qed.
...
by case defk: (split k) => /[1!mxE]; apply: eq_bigr => j _; rewrite mxE defk.
Qed.
...
Proof.
apply/matrixP=> i k /[!mxE].
by case defi: (split i) => /[1!mxE]; apply: eq_bigr => j _; rewrite mxE defi.
Qed.
...
Proof. by apply: eq_bigr=> i _ /[1!mxE]. Qed.
...
Proof. by rewrite rmorph_sum; apply: eq_bigr => i _ /[1!mxE]. Qed.
...
have{p1}: ~~ perm_on set0 p.
  ...
  by rewrite perm1; apply/eqP/idPn; move/p1 /[1!inE].
